...
# поезд может отцеплять вагоны
    def remove_wagon
        @amount_wagons -= 1 if @amount_wagons > 1 && @speed.zero?
    end
...
# пщезд может перемещаться между станциями
    def go_to_next_station
        if index_current_station == @route.list_stations.size - 1
            ...
            @current_station.trains.delete(self) # удаляем поезд со станции
            @current_station = @route.list_stations[index_current_station + 1] # Переходим на следующую станцию
            @current_station.add_train(self) # Ставим поезд на новую станцию
        end
    end
    ...
    def go_to_previos_station
        if index_current_station == 0
            ...
            @current_station.send_train(self) # удаляем поезд со станции
